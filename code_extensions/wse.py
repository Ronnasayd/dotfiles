...
def disable_extensions_json(language):
    ...
    with open(f"{curr}/.vscode/extensions.json", "w", encoding="utf-8") as file:
        extensions["recommendations"] = list(
            filter(lambda x: x not in data, extensions["recommendations"])
        )
        file.write(json.dumps(extensions))
...
def disable(language: str):
    ...
    if fetchone is not None:
        ...
        cursor.execute(
            "UPDATE ItemTable SET VALUE = ? WHERE KEY IS ?",
            (json.dumps(extensions), "extensionsIdentifiers/enabled"),
        )
        conn.commit()
        conn.close()
...
def enable(language: str):
    ...
    if fetchone is not None:
        cursor.execute(
            ...
            (extensions_json, "extensionsIdentifiers/enabled"),
        )
    else:
        cursor.execute(
            "INSERT INTO ItemTable (KEY, VALUE) VALUES (?, ?)",
            ("extensionsIdentifiers/enabled", extensions_json),
        )
