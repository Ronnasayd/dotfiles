...
def categorize_commits(commits):
    ...
    for commit_line in commits:
        ...
        clean_description = f"* {description.strip()} ({commit_hash})"
...
def generate_changelog():
    ...
    if last_tag:
        changelog_content.append(f"## v{datetime.datetime.now().strftime('%Y.%m.%d')} (Unreleased - based on {last_tag})")
        changelog_content.append("")
        commits = get_commits(since_tag=last_tag)
    else:
        print("Warning: No tags found. Generating changelog from all commits.")
        changelog_content.append(f"## v{datetime.datetime.now().strftime('%Y.%m.%d')} (Initial Release / Full History)")
        changelog_content.append("")
        commits = get_commits()
    ...
    changelog_content.append("---")
    changelog_content.append(f"Generated on {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
