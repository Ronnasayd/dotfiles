...
alias cows="cows && exit" # Open a workspace in Visual Studio Code using fzf to select from recent workspaces
...
alias replace="replace" # (Fn) replace a part of a string with a new string: cutvideo <string> <substring> <new_substring>
alias cd="z" # (Fn) Use z of zoxide to navigate to directories more easily
alias cdl="cdl" # (Fn) Jump to original directory of a symbolic link
...
alias copilot_commit="copilot_commit" # (Fn) Generate a git commit message using GitHub Copilot based on the staged changes
